...
async function loadConversations(channel = null) {
  ...
  // 명시적 리스트 엔드포인트 사용
  let url = `${API_BASE}/api/conversations/list`;
  ...
  const conversations = Array.isArray(data) ? data : (data.items || []); // [{id, customer_id, ...}]
...
async function selectConversation(conversationId, clickedItem) {
  ...
  if (messages.length === 0) {
    messagesContainer.innerHTML = '<div style="padding:20px;text-align:center;color:#999;">메시지가 없습니다.</div>';
    return;
  }
